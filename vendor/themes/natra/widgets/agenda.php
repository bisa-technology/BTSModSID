...
<div class="single_bottom_rightbar">
	<h2><a href="<?= site_url('first/kategori/1000') ?>"><i class="fa fa-calendar"></i>&ensp;<?= $judul_widget ?></a>
	</h2>
	<div id="agenda" class="box-body">
		<ul class="nav nav-tabs">
			...
			<?php if (is_array($yad) && count($yad) > 0): ?>
				<li class="<?php count($hari_ini) == 0 and print ('active') ?>"><a data-toggle="tab" href="#yad">Mendatang</a>
				</li>
			<?php endif; ?>
			<?php if (is_array($lama) && count($lama) > 0): ?>
				<li class="<?php count(array_merge($hari_ini, $yad)) == 0 and print 'active' ?>"><a data-toggle="tab"
						href="#lama">Lama</a></li>
			<?php endif; ?>
		</ul>
		<div class="tab-content">
			...
			if (is_array($merge) && count($merge) > 0):
						...
						<?php foreach ($hari_ini as $agenda): ?>
							<li>
								<table id="table-agenda" width="100%">
									<tr>
										<td colspan="3"><a
												href="<?= site_url('artikel/' . buat_slug($agenda)) ?>"><?= $agenda['judul'] ?></a>
										</td>
									</tr>
									<tr>
				...
				<div id="yad"
					class="tab-pane fade <?php is_array($hari_ini) && count($hari_ini) == 0 and print ('in active') ?>">
					<ul class="sidebar-latest">
						<?php if (is_array($yad) && count($yad) > 0): ?>
