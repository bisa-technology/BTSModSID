					...
					<div class="box-header with-border">
						<?php if (can('u')): ?>
							<div class="btn-group btn-group-vertical">
								<a class="btn btn-social btn-flat btn-success btn-sm" data-toggle="dropdown"><i
										class='fa fa-plus'></i> Tambah Penduduk</a>
								<ul class="dropdown-menu" role="menu">
									<li>
										<a href="<?= site_url('penduduk/form_peristiwa/1'); ?>"
											class="btn btn-social btn-flat btn-block btn-sm"
											title="Tambah Data Penduduk Lahir"><i class="fa fa-plus"></i> Penduduk Lahir</a>
									</li>
									<li>
										<a href="<?= site_url('penduduk/form_peristiwa/5'); ?>"
											class="btn btn-social btn-flat btn-block btn-sm"
						...
						<?php if (can('h') && !data_lengkap()): ?>
							<a href="#confirm-delete" title="Hapus Data Terpilih"
								...
								class="btn btn-social btn-flat btn-danger btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block hapus-terpilih"><i
									class='fa fa-trash-o'></i> Hapus Data Terpilih</a>
						<?php endif; ?>
						<div class="btn-group-vertical">
							<a class="btn btn-social btn-flat btn-info btn-sm" data-toggle="dropdown"><i
									class='fa fa-arrow-circle-down'></i> Pilih Aksi Lainnya</a>
							<ul class="dropdown-menu" role="menu">
								<li>
									<a id="cetak_id" href="<?= site_url("penduduk/ajax_cetak/{$p}/{$o}/cetak"); ?>"
								...
								<li>
									<a id="unduh_id" href="<?= site_url("penduduk/ajax_cetak/{$p}/{$o}/unduh"); ?>"
								...
								<li>
									<a href="<?= site_url('penduduk/program_bantuan') ?>"
										class="btn btn-social btn-flat btn-block btn-sm"
										title="Pencarian Program Bantuan" data-remote="false" data-toggle="modal"
										data-target="#modalBox" data-title="Pencarian Program Bantuan"><i
											class="fa fa-search"></i> Pencarian Program Bantuan</a>
								</li>
								<li>
									<a href="<?= site_url('penduduk/search_kumpulan_nik'); ?>"
										class="btn btn-social btn-flat btn-block btn-sm" title="Pilihan Kumpulan NIK"
										data-remote="false" data-toggle="modal" data-target="#modalBox"
										data-title="Pilihan Kumpulan NIK"><i class="fa fa-users"></i> Pilihan Kumpulan
										NIK</a>
								</li>
								<li>
									<a href="<?= site_url("{$this->controller}/nik_sementara"); ?>"
										class="btn btn-social btn-flat btn-block btn-sm" title="NIK Sementara"><i
											class="fa fa-search"></i> NIK Sementara</a>
								</li>
								<li>
						...
						<div class="btn-group-vertical">
							<a class="btn btn-social btn-flat bg-navy btn-sm" data-toggle="dropdown"><i
									class='fa fa-arrow-circle-down'></i> Impor / Ekspor</a>
							<ul class="dropdown-menu" role="menu">
								<?php if (!config_item('demo_mode') && auth()->id_grup == $akses && !data_lengkap()): ?>
									<li>
										<a href="<?= route('penduduk.impor') ?>"
											class="btn btn-social btn-flat btn-block btn-sm" title="Impor Penduduk"><i
												class="fa fa-upload"></i> Impor Penduduk</a>
									</li>
									<?php if (!setting('multi_desa')): ?>
										<li>
											<a href="<?= route('penduduk.impor_bip') ?>"
												class="btn btn-social btn-flat btn-block btn-sm" title="Impor BIP"><i
													class="fa fa-upload"></i> Impor BIP</a>
										</li>
									<?php endif ?>
								<?php endif ?>
								<li>
									<a href="<?= route('penduduk.ekspor') ?>" target="_blank"
										class="btn btn-social btn-flat btn-block btn-sm" title="Ekspor Penduduk"><i
											class="fa fa-download"></i> Ekspor Penduduk</a>
								</li>
							</ul>
						</div>
						<a href="<?= site_url("{$this->controller}"); ?>"
							class="btn btn-social btn-flat bg-purple btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block"><i
								...
								<div class="row">
									<div class="col-sm-9">
										...
										<select class="form-control input-sm" name="sex"
											onchange="formAction('mainform', '<?= site_url('penduduk/filter/sex'); ?>')">
									...
									<div class="col-sm-3">
										<div class="input-group input-group-sm pull-right">
											<input name="cari" id="cari" class="form-control" placeholder="Cari..."
												type="text" title="Pencarian berdasarkan nama penduduk"
												value="<?= html_escape($cari); ?>"
												onkeypress="if (event.keyCode == 13){$('#'+'mainform').attr('action', '<?= site_url('penduduk/filter/cari'); ?>');$('#'+'mainform').submit();}">
											<div class="input-group-btn">
												<button type="submit" class="btn btn-default"
													onclick="$('#'+'mainform').attr('action', '<?= site_url('penduduk/filter/cari'); ?>');$('#'+'mainform').submit();"><i
									...
									<table
										...
										<thead class="bg-gray disabled color-palette">
											<tr>
												...
												<th>Aksi</th>
												<th>Foto</th>
												<th><?= url_order($o, "{$this->controller}/{$func}/1", 1, 'NIK'); ?>
												</th>
												<th>Tag ID Card</th>
												<th><?= url_order($o, "{$this->controller}/{$func}/1", 3, 'Nama'); ?>
												</th>
												<th><?= url_order($o, "{$this->controller}/{$func}/1", 5, 'No. KK'); ?>
												</th>
												...
												<th><?= url_order($o, "{$this->controller}/{$func}/1", 7, 'Umur'); ?>
												</th>
												<th>Pekerjaan</th>
												<th>Kawin</th>
												<th><?= url_order($o, "{$this->controller}/{$func}/1", 11, 'Tgl Peristiwa'); ?>
												</th>
												<th><?= url_order($o, "{$this->controller}/{$func}/1", 9, 'Tgl Terdaftar'); ?>
												</th>
											</tr>
										</thead>
										<tbody>
											<?php if ($main): ?>
												<?php foreach ($main as $key => $data): ?>
													<tr <?= valid_nik_nokk($data['nik']) ?>>
																...
																<ul class="dropdown-menu" role="menu">
																	...
																	<?php if ($data['status_dasar'] == 9 && can('u')): ?>
																		<li>
																			<a href="#"
																				...
																				data-body="Apakah Anda yakin ingin mengembalikan status data penduduk ini?<br> Perubahan ini akan mempengaruhi laporan penduduk bulanan."><i
																					class="fa fa-undo"></i> Kembalikan ke Status
																				HIDUP</a>
																		</li>
																	<?php endif; ?>
																	<?php if ($data['status_dasar'] == 1): ?>
																		<?php if (can('u')): ?>
																			...
																			<li>
																				<a href="<?= site_url("penduduk/ajax_penduduk_maps/{$p}/{$o}/{$data['id']}/0"); ?>"
																					class="btn btn-social btn-flat btn-block btn-sm"><i
																						class='fa fa-map-marker'></i> Lihat Lokasi
																					Tempat Tinggal</a>
																			</li>
																			<?php if (data_lengkap()): ?>
																				<li>
																					<a href="<?= site_url("penduduk/edit_status_dasar/{$p}/{$o}/{$data['id']}"); ?>"
																						...
																						class="btn btn-social btn-flat btn-block btn-sm"><i
																							class='fa fa-sign-out'></i> Ubah Status
																						Dasar</a>
																				</li>
																			<?php endif; ?>
																		<?php endif; ?>
														...
														<td class="padat">
															<a href="<?= site_url("penduduk/ambil_foto?foto={$data['foto']}&sex={$data['id_sex']}"); ?>"
																class="progressive replace penduduk_kecil">
																<img class="preview " loading="lazy"
																	src="<?= asset('images/img-loader.gif') ?>"
																	alt="Foto Penduduk" />
															</a>
														...
														<td>
															<a href="<?= site_url("penduduk/detail/{$p}/{$o}/{$data['id']}"); ?>"
																id="test" name="<?= $data['id']; ?>"><?= $data['nik']; ?></a>
														</td>
														...
														<!-- tambah kolom orang tua-->
														<td><a
																href="<?= site_url("rtm/anggota/{$data['id_rtm']}"); ?>"><?= $data['no_rtm']; ?></a>
														</td>
